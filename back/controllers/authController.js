...
exports.registroUsuario = catchAsyncErrors(async (req, res, next) => {
  ...
  const result = await cloudinary.v2.uploader.upload(req.body.avatar, {  //para diligenciar los avatar
...
exports.forgotPassword = catchAsyncErrors ( async( req, res, next) =>{
  ...
  try{
      await sendEmail({
          email:user.email,
          subject: "WA Galeria Recuperación de la contraseña",
          mensaje
      })
      res.status(200).json({
          success:true,
      ...
      await user.save({validateBeforeSave:false}); //validateBeforeSaveNo me valide para hacer esta opción
...
exports.resetPassword = catchAsyncErrors(async (req,res,next) =>{
  ...
  const user= await User.findOne({
      resetPasswordToken,
      resetPasswordExpire:{$gt: Date.now()} //verificando que la fecha de expiración del token sea posterior al momemnto actual
  })
  ...
  user.resetPasswordToken=undefined;  //vaciamos el token buena práctica
  user.resetPasswordExpire=undefined; //vaciamos el tiempo buena práctica
...
exports.updatePassword= catchAsyncErrors(async (req, res, next) =>{
  const user= await User.findById(req.user.id).select("+password"); //+password traemos la contraseña con un plus
...
exports.updateProfile= catchAsyncErrors(async(req,res,next)=>{
  //Actualizar el email por user a decisiòn de cada uno OJO
  const newUserData ={
  ...
  const user = await User.findByIdAndUpdate(req.user.id, newUserData, { //findByIdAndUpdate busca por id y luego lo actualiza
      new:true,
      runValidators:true,
